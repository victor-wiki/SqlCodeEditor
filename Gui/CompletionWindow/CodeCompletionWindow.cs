...
namespace SqlCodeEditor.Gui.CompletionWindow
{
	public class CodeCompletionWindow : AbstractCompletionWindow
	{
        ...
        public static CodeCompletionWindow ShowCompletionWindow(Form parent, TextEditorControl control, SqlWordToken[] sqlWordTokens, ICompletionDataProvider completionDataProvider, char firstChar, bool showDeclarationWindow, bool fixedListViewWidth)
        {
            ICompletionData[] completionData = completionDataProvider.GenerateCompletionData(sqlWordTokens, control.ActiveTextAreaControl.TextArea, firstChar);
            if (completionData == null || completionData.Length == 0)
            {
                return null;
            }
